...
class ChangePasswordActivity : AppCompatActivity() {
    ...
    private fun changePassword(oldPassword:String, newPassword:String, bForgotPassword: Boolean): String {
        val prefs = getSharedPreferences("co.anode.anodium", Context.MODE_PRIVATE)
        val jsonObject = JSONObject()
        var url = apiChangePasswordUrl.replace("<username>", prefs.getString("username", "")!!, true)
        if (!bForgotPassword) {
            jsonObject.accumulate("currentPassword", oldPassword)
        } else {
    ...
    private fun changePasswordHandler(result:String) {
